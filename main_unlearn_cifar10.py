...
from resnet_cifar100 import ProjectionMLP, ModelWrapper
...
from utils import (
    ...
    estimate_structure_aware_importance,
    naive_train,
    test,
    test_ours,
)
...
def main():
    ...
    for unlearn_k in k_arr:
        ...
        save_dir = 'CIFAR10_unlearned_model'
        ...
        model.load_state_dict(torch.load('pretrained_models/cifar10_pretrained_models/resnet18.pt', map_location=device))
        normalize_layer = NormalizeLayer(device, (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
...
###############################################################################################################################################################
        ...
        model_path = os.path.join(save_dir, f'CIFAR10_{unlearn_k}_Naive_unlearned_model.pt')   
...
################################################################################################################################################################
        ...
        model = resnet18().to(device)
        model.load_state_dict(torch.load('pretrained_models/cifar10_pretrained_models/resnet18.pt'))
        normalize_layer = NormalizeLayer(device, (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        model = torch.nn.Sequential(normalize_layer, model)
        ...
        model_path = os.path.join(save_dir, f'CIFAR10_{unlearn_k}_{args.base_pgd_eps}_Adv_unlearned_model.pt')   
        torch.save(model.state_dict(), model_path)
...
###############################################################################################################################################################
        ...
        model_path = os.path.join(save_dir, f'CIFAR10_{unlearn_k}_{args.base_pgd_eps}_Adv_Weight_unlearned_model.pt')   
        torch.save(model.state_dict(), model_path)
...
##############################################################################################################################################################################        
        ...
        with open("CIFAR10_Description.json", "r") as f:
        ...
        projection_mlp = ProjectionMLP(input_dim=512, hidden_dim=512, output_dim=10).to(device)
        ...
        while unlearn_acc != 0:
            ...
            for i , data in enumerate(zip(adv_loader, unlearn_loader_cycle)):
                ...
                if i == 1 and not already_computed_importance: 
                        importance = estimate_structure_aware_importance(adv_loader, copy.deepcopy(projection_mlp), model_for_importance, model, device, num_samples, semantic_anchors, args.weight_level)    
        ...
        model_path = os.path.join(save_dir, f'CIFAR10_{unlearn_k}_{args.pgd_eps}_Ours_unlearned_model.pt')   
        torch.save(model.state_dict(), model_path)
